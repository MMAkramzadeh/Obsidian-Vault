...
#### Zip() Function
- you can link two list together with the same indexes:
```Py
names = ["James", "John", "Tony"]
lastNames = ["Bond", "Wick", "Stark"]

print(Zip(names, lastnames))
```
> [("James", "Bond"), ("John", "Wick"), ("Tony", "Stark")]

## List comprehension
- Its a good way to turn this code:
```py
even_numbers = []
...
```
- filter is a function that accepts a function as its first argument and a list as its second argument. it then takes a member of the list and passes it into the function and if the function returns true then it will be return from the filter function.

#### map() Function
```py
elsius = [0, 10, 20, 30, 40]
...
```
- Works exactly like the filter() Function but the only difference is that its not about it being true or false and it just gives out the output.
...
## Lambda Functions
- Functions with no name that are called immediately when ran:
```py
def square(num):
	return num ** 2
...
lambda num: num ** 2
```
- its really good for passing into map or filter like this:
```py
numbers = [1, 2, 3, 4, 5]
